...
# Step 1: Identify students who missed at least one MCQ_xx grade
students_missed_MCQ <- grades %>%
  rowwise() %>%
  mutate(Missed = any(is.na(c_across(starts_with("MCQ"))))) %>%
  ungroup()

# Step 2: Calculate the percentage of students in each group who missed at least one MCQ_xx grade
...
# Show the table with 'Group' and 'P_missed' columns
...
```{r percent_scatter}
# Identify students who missed at least one MCQ_xx grade
students_missed_MCQ <- grades %>%
  rowwise() %>%
  mutate(Missed = any(is.na(c_across(starts_with("MCQ"))))) %>%
  ungroup()

# Calculate the percentage of students in each group who missed at least one MCQ_xx grade
group_missed_percentage <- students_missed_MCQ %>%
  group_by(Group) %>%
  summarise(Total_Students = n(),
            Missed_Count = sum(Missed, na.rm = TRUE),
...
# Calculate the average Exam grade for each group
average_exam_by_group <- grades %>%
  group_by(Group) %>%
  summarise(Avg_Exam_Grade = mean(Exam, na.rm = TRUE))

# Merge the two intermediate tables
