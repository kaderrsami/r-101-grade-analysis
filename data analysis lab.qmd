...
# Step 1 & Step 2: Calculate missing counts and merge with original data in one go
merged_df <- grades %>%
  rowwise() %>%
...
# Step 3 & Step 4: Filter rows and create Exam_status in one go
filtered_df <- merged_df %>%
  filter((missing_exam == missing_online_tests) | (missing_exam == 0)) %>%
  mutate(Exam_status = ifelse(is.na(Exam), "Missed Exam", "Taken Exam"))

# Count missed exams and total students missing online tests in one go
